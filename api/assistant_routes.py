...
async def search_keyword(req: AssistantRequest):
    ...
    system_prompt = (
        ...
        "scanner tous.\n\n"
        "VOCABULAIRE TEMPOREL — utilise ces mappings pour months_back :\n"
        "  - 'aujourd'hui', 'cette semaine', 'cette semaine-ci' → 1\n"
        "  - 'ce mois-ci', 'le mois passé', 'le mois dernier' → 2\n"
        "  - 'récemment', 'récent', 'dernièrement', 'ces derniers temps' → 3\n"
        "  - 'depuis quelques mois', 'il y a quelques mois' → 6\n"
        "  - 'cette année', 'cette année-ci', 'depuis le début de l'année' → 12\n"
        "  - 'l'année passée', 'l'an dernier' → 18\n"
        "  - 'il y a un bon moment', 'il y a longtemps' → 36\n"
        "  - 'depuis toujours', 'jamais', 'historique complet' → 60\n"
        "  - aucun qualificatif temporel mentionné → laisse months_back vide "
        "(le backend mettra 24 par défaut)\n\n"
