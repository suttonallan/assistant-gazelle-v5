...
>   - P3 ÉVÉNEMENTS PERSO : `_build_personal_entry` (kind=personal, heure/« Journée », titre, sans IA) ; orchestrateur affiche les PERSONAL/HOLIDAY SANS client (§7 M6 : avec client = flux normal) ; BriefingCard rend une carte « Perso » lecture seule. VÉRIFIÉ end-to-end (épicerie 2027 ressort ; build frontend OK).
> RESTE : Phase 2 (récence/supersession/PLS — ATTENTION §7 G1 liste blanche stricte = le plus dangereux ; G2 polarité REPAIR ; M3 couverture PLS) ET Phase 4 (provenance frontend) ET Phase 5 (cron qualité) — ces 3 reposent sur le PIPELINE DE FAITS (nouveau sous-package `modules/briefing/facts/*` + réécriture `NARRATIVE_PROMPT` + sortie hybride, §2). + B2 (flag/version clé cache, purge au flip) AVANT toute bascule A/B. C'est le gros chantier d'architecture — à faire en session focalisée, G1 avec un soin maximal.
> Bloqué par : rien
>
